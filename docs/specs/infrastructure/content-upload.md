...
4. ファイルサイズが上限以内であることを確認する
5. ファイルコンテンツをバイト列として読み取る
6. Local インジェスターの取り込み処理を呼び出す（`upload_mode` を引き渡す）
7. パイプラインを実行し、インジェストロックを解放する
...
3. ファイル名をサニタイズし、拡張子を検証する
4. ファイルサイズが上限以内であることを確認する
5. ファイルコンテンツを UTF-8 テキストとして読み取る
6. Journal インジェスターの取り込み処理を呼び出す（`title`、`repository`、`entry_id` を引き渡す）
7. パイプラインを実行し、インジェストロックを解放する
...
4. ファイルコンテンツをバイト列として読み取る（コンテンツデコードは不要）
...
4. ファイルコンテンツを UTF-8 テキストとして読み取る
