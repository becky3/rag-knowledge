...
- **小さいファイルは既定で対象外**: 削減は画像の非可逆な再エンコードを伴い、小さいファイルでは画質を落とす割に得られる削減量がごくわずかなため、既定では一定サイズ以上のファイルのみを対象とする。下限は `--min-size-mb` で変更でき、0 指定で全ファイルを対象にできる。既定値はコード定義を SSoT とする
