...
- **非破壊**: 入力ファイルは一切変更しない。加工はメモリ上でのみ行い、結果は別ファイルとして出力する。in-place 上書きモードは設けない
...
- **小さい画像は対象外**: 一定サイズ未満の画像は、デコードと再エンコードのコストに見合う削減が得られないため対象としない。判定は透過マスクを含めた合計サイズで行う。閾値はコード定義を SSoT とする
- **小さいファイルは既定で対象外**: 削減は画像の非可逆な再エンコードを伴い、小さいファイルでは画質を落とす割に得られる削減量がごくわずかなため、既定では一定サイズ以上のファイルのみを対象とする。
  実測（発表資料 PDF 495 件）では、下限未満の帯はファイルの 5〜7 割が削減後も残るのに対し、下限以上の帯は 1〜3 割まで縮む。下限は `--min-size-mb` で変更でき、0 指定で全ファイルを対象にできる。既定値はコード定義を SSoT とする
- **ストリーミング処理ではない**: pptx 版と異なり、画像を 1 枚ずつデコード・再エンコードするため、処理時間とメモリ使用量は入力の画像量に比例する。zip エントリのコピーで完結する pptx 版とはコスト特性が異なる
- **画像は 1 枚ずつ差し替える**: PDF ライブラリが提供する一括再圧縮機能は使用しない。同一入力でも非決定的にプロセスごとクラッシュすることを実測で確認しており、既知の上流不具合として報告されている。クラッシュしなかった実行でも誤った値を参照している可能性があるため結果を信頼できない。画像単位の取り出し・縮小・差し替えを自前で行い、1 枚の失敗が処理全体を止めない構造とする
...
- **表示サイズは 1 ページ分のみ調べる**: 実効解像度の算出に必要な表示矩形は、その画像を最初に参照するページ 1 枚から求める。全ページを走査して最大の配置を探すと、ページ数 × 画像数の組み合わせでコンテンツストリームの走査が走り、処理時間の大半を占めるため。矩形を取得できない場合はページ全面を上限として扱い、縮小率を安全側（粗くなりすぎない方向）に倒す
...
| `--min-size-mb` | 整数 | いいえ | このサイズ（MB）未満のファイルを対象から除外する。0 で除外なし。既定値はコード定義を SSoT とする |
| `--jobs` | 整数 | いいえ | 並列に処理するファイル数（1 で直列実行）。処理は画像のデコード・再エンコードが支配的な CPU バウンドのため、ファイル単位の並列化が短縮に効く。既定値はコード定義を SSoT とする |
...
1. `paths` から対象ファイルを収集する（ディレクトリは再帰走査し `.pdf` をフィルタ。`.reduced` サフィックス付きは除外）。サイズ下限（`--min-size-mb`）未満のファイルを対象から外し、除外件数を表示する
2. 出力先パス（`--output-dir/<入力ファイル名>` または `--alongside` 時は `<原本と同じフォルダ>/<元名>.reduced.pdf`）が同一実行内で衝突する場合、または同名ファイルが既に存在する場合は警告してスキップする（PDF を開く前に判定し、実行対象から外す）。残った対象をファイル単位で並列処理する（`--jobs`。1 ファイルの失敗は当該ファイルのエラー計上に留まり、他のファイルに影響しない。結果の表示は入力順を保つ）
3. 各ファイルについて削減対象の占有量を解析し、テキスト層の乏しさを判定してレポート表示する。解析は削減と同じ 1 回の読み込みで行う（レポートのために開き直すと画像の取り出しと表示矩形の解決が二重に走るため）
4. `--report-only` の場合はここで終了する
5. テキスト層が乏しく `--include-scanned` が未指定の場合は、理由を表示してスキップする
...
文書全体から等間隔にサンプリングしたページのテキスト量が 1 ページあたりの下限を下回る場合、テキスト層が乏しいと判定する。判定の観点は PDF テキスト抽出のバックエンド事前判定（[../converter.md](../converter.md)）と同じで、テキスト層を持たない PDF は OCR 経路で処理されるため画像が抽出品質に直結する。

サンプル数と下限は、PDF テキスト抽出の事前判定と同じ設定値を用いる。同じ観点の判定を二重に定義すると、値がずれたときに「抽出側と同じ基準で判定している」という前提が崩れるため。
...
| 0 バイトの PDF・壊れファイル | エラー計上し、スキップして続行する |
| パスワード保護された PDF | エラー計上し、スキップして続行する。この形式は開く操作自体は成功し、内容へアクセスした時点で初めて失敗するため、開いた直後に保護の有無を判定する |
...
| 画像の表示矩形を取得できない（リソースに登録されているが描画されていない等） | ページ全面に配置されたものとして扱う（実効解像度を低めに見積もり、縮小を保守的にする） |
