...
- **表示サイズは 1 ページ分のみ調べる**: 実効解像度の算出に必要な表示矩形は、その画像を最初に参照するページ 1 枚から求める。全ページを走査して最大の配置を探すと、ページ数 × 画像数の組み合わせでコンテンツストリームの走査が走り、100 ページ規模の資料で 1 ファイルあたり数百秒を要して処理時間の大半を占める。矩形を取得できない場合はページ全面を上限として扱い、縮小率を安全側（粗くなりすぎない方向）に倒す
