...
- `0 = 無制限` のセマンティクスを排除する。全制限値に下限を定義し、0 や負数を拒否する
...
| 生 HTTP クライアント利用禁止 | CI チェック | ConstrainedClient 経由を強制 | 許可例外は `# safety:allowed` コメントで可 |
...
| ツール | 入力 | 振る舞い |
|--------|------|---------|
| rag_crawl_zenn | username、max_articles（任意） | 指定ユーザーの Zenn 記事を API 経由で取得し、ナレッジベースに取り込む。同一記事（`slug` ベース）の再取り込み時は既存の知識を最新に置き換える |
...
4. IngestedContent を構築して返す:
   - `source_id`: `https://zenn.dev/{path}`（記事の公開 URL）
   - `title`: 記事タイトル
   - `text`: 抽出済みテキスト
   - `source_type`: `"zenn"`
...
| `max_articles` に 0 や負数を指定 | バリデーションエラーとして拒否する |
