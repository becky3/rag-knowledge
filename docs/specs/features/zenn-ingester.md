...
- 上限解除には CLI オプション `--no-limit` の明示的な指定を要求する。MCP ツール経由では上限解除できない（AI エージェントが人間の確認なしに上限を解除することを防ぐため）
- 上限到達時は取得済みデータを返し、上限に達した旨をログに出力する（WARNING レベル）

### 個別記事取得の件数上限

- 一括取り込み時に個別取得する記事数の上限: **100 件**
- ページネーションで発見された記事が 100 件を超える場合、先頭 100 件のみ取得する
- 上限到達時は取得済みデータを返し、上限に達した旨をログに出力する（WARNING レベル）
- `--no-limit` 指定時はこの上限も解除される
...
| 操作 | トリガー | 概要 |
| --- | --- | --- |
| Zenn 記事一括取り込み | MCP ツール / CLI | 指定ユーザーの記事を API 経由で一括取得し、ナレッジベースに取り込む。MCP ツール経由では上限解除不可 |
...
| `--no-limit` | いいえ | ページネーション上限・記事数上限を解除する（CLI のみ。MCP ツールでは使用不可） |
...
| 記事数上限到達 | 発見された記事が 100 件を超える場合、先頭 100 件のみ取得し、上限に達した旨をログに WARNING で出力する |
