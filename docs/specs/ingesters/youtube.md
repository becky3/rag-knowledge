...
| 最悪ケース所要時間 | 字幕あり: 数秒。Whisper フォールバック: 音声DL時間 + 文字起こし時間（動画長依存）。操作全体タイムアウト 600 秒の範囲内 |
...
| 最悪ケース所要時間 | 499 × 5.0 秒（デフォルト間隔、動画間のみ待機）≈ 2495 秒 + 字幕取得/Whisper 処理時間。プレイリスト一括取り込みは最大 500 動画 × リクエスト間隔で長時間かかるため、操作全体タイムアウトは設けず、動画単位タイムアウト + サーキットブレーカーで安全性を担保する |
...
| 取得動画数上限 | ハードリミット | 500 件 | 引き上げ不可（引き下げ可） |
| 最低リクエスト間隔 | ハードリミット | 0.1 秒 | 引き下げ不可（引き上げ可） |
| 音声ファイルサイズ上限 | ハードリミット | 500 MB | 引き上げ不可 |
| 操作全体タイムアウト | — | なし（動画単位タイムアウト + サーキットブレーカーで安全性を担保） | — |
...
| リクエスト間隔 | 設定値 | 許容範囲 0.1〜60 秒、デフォルト 5.0 秒 | 範囲内で変更可 |
...
| `max_videos` | 整数 | いいえ | 取得する最大動画数。デフォルト: 100、許容範囲: 1〜500 |
...
| `rag_youtube_whisper_model` | 文字列 | `.env` | `"base"` | `"tiny"` / `"base"` / `"small"` / `"medium"` / `"large-v3"` 等 | faster-whisper のモデル名 |
| `rag_youtube_whisper_device` | 文字列 | `.env` | `"cuda"` | `"cuda"` / `"cpu"` | faster-whisper のデバイス |
| `rag_youtube_transcript_languages` | リスト | `config.toml` | `["ja", "en"]` | 言語コードのリスト | 字幕取得の優先言語 |
| `rag_youtube_max_duration` | 整数 | `config.toml` | 14400 | 60〜86400 | 動画長上限（秒） |
...
| API エラー（IP ブロック、リクエスト失敗等） | Whisper にフォールバック**しない**。エラーログに記録してスキップする。一時的エラーで品質の低い Whisper 結果が生成されることを防ぐ |
...
| channel_id が取得できない場合 | エラーログに記録してスキップする（ディレクトリ構成がチャンネル別のため、channel_id なしでは配置不可） |
