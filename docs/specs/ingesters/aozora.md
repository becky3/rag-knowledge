...
| パラメータ | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| `book_id` | 文字列 | はい | 青空文庫の作品 ID（CSV の作品 ID カラムに対応）。前後空白を除去し、6 桁未満はゼロ埋め、6 桁以上はそのままカタログ検索に使う（数値検証は行わない。詳細は [URL 変換規則](#url-変換規則) を参照） |
...
| パラメータ | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| `person_id` | 文字列 | はい | 著者の人物 ID（`rag_search_aozora` で確認可能）。前後空白を除去し、6 桁未満はゼロ埋め、6 桁以上はそのままカタログ検索に使う（数値検証は行わない。詳細は [URL 変換規則](#url-変換規則) を参照） |
| `max_works` | 整数 | いいえ | 取得する最大作品数 |
...
| 設定項目 | 層 | 設計意図 |
|---------|-----|---------|
| `rag_aozora_max_works` | 共通設定値 | 一括取り込み時の最大作品数。API 負荷を抑制 |
| `rag_aozora_request_timeout` | 共通設定値 | リクエストのタイムアウト |
| `rag_aozora_request_interval` | 共通設定値 | リクエスト間の最低間隔。GitHub Raw URL のレート制限を考慮 |
...
- `person_id`: 著者 ID（カタログ CSV の「人物ID」カラム値は 6 桁ゼロ埋め文字列、例: `000035`）。URL 内でも 6 桁ゼロ埋め形式で現れる
- `book_id`: 作品 ID（カタログ CSV の「作品ID」カラム値は 6 桁ゼロ埋め文字列、例: `001567`）。一方、URL の filename 部分（`{book_id}_{file_id}.html`）ではゼロ埋めなしの数値部分（例: `1567`）として現れることがある（CSV の `XHTML/HTMLファイルURL` カラム値に由来する）
- `file_id`: ファイル ID（`14913` 等、CSV の `XHTML/HTMLファイルURL` カラム値から取得）

インジェスター入口（`add_work` / `crawl_author`）では `str.strip().zfill(6)` により短桁入力を 6 桁ゼロ埋めに正規化し、カタログ CSV の「作品ID」「人物ID」カラム値（6 桁ゼロ埋め形式）と照合する。
...
6 桁未満はゼロ埋め、6 桁以上はそのまま検索に使う（数値検証や桁数上限チェックは行わない。不正入力はカタログ検索の「見つかりません」エラーに収束する）。
...
| `book_id` / `person_id` に 1〜6 桁の数字文字列を指定 | `str.strip().zfill(6)` で 6 桁ゼロ埋めに正規化してカタログ検索する（6 桁入力は冪等で無変更） |
| `book_id` / `person_id` に 7 桁以上の数字文字列を指定 | zfill 効果なしでそのままカタログ検索する（数値検証・桁数上限チェックなし、不正値はカタログ検索の「見つかりません」エラーに収束） |
| `book_id` / `person_id` に非数字文字（例: `abc`）を指定 | バリデーションは行わず、カタログ検索の「見つかりません」エラーに収束する |
