...
| パラメータ | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| `book_id` | 文字列 | はい | 青空文庫の作品 ID（CSV の作品 ID カラムに対応）。1〜6 桁の数字文字列を許容し、内部で 6 桁ゼロ埋めに正規化される（詳細は [URL 変換規則](#url-変換規則) を参照） |
...
| パラメータ | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| `person_id` | 文字列 | はい | 著者の人物 ID（`rag_search_aozora` で確認可能）。1〜6 桁の数字文字列を許容し、内部で 6 桁ゼロ埋めに正規化される（詳細は [URL 変換規則](#url-変換規則) を参照） |
...
- `person_id`: 著者 ID（6 桁ゼロ埋め文字列、例: `000035`）
- `book_id`: 作品 ID（6 桁ゼロ埋め文字列、例: `001567`）
- `file_id`: ファイル ID（`14913` 等、CSV から取得）

`person_id` および `book_id` の正規形は 6 桁ゼロ埋め文字列であり、source_id・カタログ CSV のカラム値・URL 変換いずれもこの形式を前提とする。
インジェスター入口（`add_work` / `crawl_author`）では `str.strip().zfill(6)` により 1〜6 桁の数字文字列を 6 桁ゼロ埋めに正規化する。
...
数値検証や桁数上限チェックは行わない（不正入力はカタログ検索の「見つかりません」エラーに収束する）。
...
| `book_id` / `person_id` に 1〜5 桁の数字文字列を指定 | `str.strip().zfill(6)` で 6 桁ゼロ埋めに正規化してカタログ検索する |
