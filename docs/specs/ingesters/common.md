...
- **契約**: 呼び出すと ConstrainedClient 経由で HTTP GET を実行し、レスポンスが非 2xx の場合は `httpx.HTTPStatusError` を送出する
...
- **リダイレクト追従との関係**: リダイレクト追従を伴う HTTP GET（bluesky の HLS プレイリスト・セグメント取得等）は媒体固有ヘルパーで実装する。httpx の `raise_for_status` は 3xx でも例外を送出するため、リダイレクト追従ヘルパー内では `fetch_get` を使わず、非 3xx 応答に到達した時点で `raise_for_status` を呼ぶ実装とする。こうすることで 3xx は追従ロジックに回り、2xx は正常返却、4xx/5xx は例外化される
- **site-ingest (web) の扱い**: Scrapy subprocess 経由で独自の HTTP クライアントを持つため本ヘルパーの対象外。Scrapy のミドルウェア（HttpError）がステータスチェックを担う
- **媒体ごとの例外ハンドリング方針**: 各媒体仕様書（[bluesky.md](bluesky.md) / [zenn.md](zenn.md) / [aozora.md](aozora.md)）のエッジケースセクションを参照。代表的な方針: API の単発失敗は該当アイテムをスキップして処理継続、カタログ・一覧取得の失敗は中断（回復不能のため）
