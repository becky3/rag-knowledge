...
- **リダイレクト追従との関係**: リダイレクト追従を伴う HTTP GET（bluesky の HLS プレイリスト・セグメント取得等）は媒体固有ヘルパーで実装する。`fetch_get` は 3xx も例外化する契約のため、リダイレクト追従ヘルパー内では `fetch_get` を使わず、リダイレクト対象ステータス以外に到達した時点で `raise_for_status` を呼ぶ実装とする。こうすることで 301/302/307/308 は追従ロジックに回り、2xx は正常返却、それ以外の 3xx・4xx・5xx は例外化される
