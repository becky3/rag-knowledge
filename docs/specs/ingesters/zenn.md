...
レスポンスにはスクラップ一覧の全フィールドに加え、以下が含まれる:

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `comments` | 配列 | コメントオブジェクトの配列 |
...
| ケース | 振る舞い |
|--------|---------|
| ユーザーが存在しない | API が空の配列を返す。0 件取得として正常終了する |
| ユーザーの記事・スクラップが 0 件 | 空の配列。0 件取得として正常終了する |
...
| スクラップのコメントが 0 件 | 該当スクラップをスキップする。コメントのないスクラップはコンテンツがないため |
| スクラップのコメント `body_html` が全て空 | 該当スクラップをスキップする |
