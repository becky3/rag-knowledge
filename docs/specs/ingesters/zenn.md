...
| `max_articles` | 整数 | いいえ | 取得する最大コンテンツ数 |
...
| 設定項目 | 層 | 設計意図 |
|---------|-----|---------|
| `rag_zenn_max_articles` | 共通設定値 | 記事・スクラップそれぞれの取得上限。API 負荷を抑制 |
| `rag_zenn_request_timeout` | 共通設定値 | Zenn API リクエストのタイムアウト。記事・スクラップ両方に適用 |
| `rag_zenn_request_interval` | 共通設定値 | Zenn API リクエスト間の最低間隔。記事・スクラップ両方に適用 |
...
| 記事・スクラップ詳細取得に失敗（404 等） | 該当アイテムをスキップし、他のアイテムの処理を続行する。`errors` に `metadata_fetch` カテゴリで計上してエラーをログ出力する |
| 記事一覧 API のレスポンス形式変更 | JSON パースエラーまたは必須フィールド欠落として処理を中断する。エラーの詳細をログ出力する |
| 記事・スクラップ詳細 API のレスポンス形式変更 | 該当アイテムをスキップし、他のアイテムの処理を続行する。`errors` に `metadata_fetch` カテゴリで計上し、エラーの詳細をログ出力する |
...
| 同一記事の再取り込み（`force` 指定時） | source_id（ファイルパス）に対応する既存ファイルを上書きする。`overwritten` に計上する（`placed` には計上しない。[common.md](common.md) の「`placed` と `overwritten` の排他関係」参照） |
