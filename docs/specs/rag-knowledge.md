...
MCP サーバーとして独立動作し、7 つのツールを提供する。

スコープ:

- 知識の取り込み（クロール・単一ページ追加・Zenn 記事取り込み）
...
見出しチャンカー・テーブルチャンカーは、入力ファイルの形式に応じて構文認識を切り替える。

#### モード分岐

ファイル拡張子に基づいてモードを決定する。
...
| テーブル | `\|===` 開閉デリミタ | `\|---\|` 区切り | テーブルチャンカー |
| コードブロック | `----` デリミタ | ` ``` ` | 見出しチャンカー（分割防止） |
...
| テーブルチャンカー | テーブルデータを行単位で分割し、各チャンクにヘッダー行を付加する。Markdown テーブルと AsciiDoc テーブル（`\|===` デリミタ）に対応する |
| 見出しチャンカー | 見出し単位で分割し、親見出しの階層情報を保持する。Markdown 見出し（`#`）と AsciiDoc 見出し（`==`）に対応する |
