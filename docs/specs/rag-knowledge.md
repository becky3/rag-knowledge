...
- **サーバーログのフォーマット**: `rag` 名前空間ロガーに設定した StreamHandler / ファイルハンドラの両方に、以下のフォーマッタを適用する:

  ```
  [MCP] %(asctime)s - %(name)s - %(levelname)s - %(message)s
  ```

  先頭の `[MCP]` プレフィックスは、同一ファイル内に混在する CLI 転送行（`[CLI]` プレフィックス）と視覚的に区別するためのもの。フォーマット文字列は設計上変更しない固定値とする
...
- **転送経路**: CLI stderr 行は、ロガー（`logger.debug` 等）を経由せず、`rag` 名前空間ロガーの各ハンドラに直接 emit する。その際、ハンドラのフォーマッタを一時的にパススルー用（`%(message)s`）に差し替え、元のフォーマッタに戻す
- **出力フォーマット**: `[CLI]` プレフィックス（半角スペース 1 つ区切り）を付与した CLI 側のフォーマット済み行をそのまま出力する。CLI 側のタイムスタンプ・ロガー名・レベルはすでに CLI のフォーマッタで整形済みのため、ファイル上は以下の形となる:

  ```
  [CLI] YYYY-MM-DD HH:MM:SS,mmm - logger.name - LEVEL - message
  ```

- **Why（この設計の理由）**:
  - `logger.debug` 経由で `[CLI] ...` を転送すると、サーバー側のフォーマッタ（`[MCP] ...`）が適用され、CLI 側で既に付与済みのタイムスタンプ・ロガー名・レベルと二重になる
  - ハンドラに直接 emit + フォーマッタ一時差し替えにより、CLI 行の整形状態を保ったまま `SessionRotatingFileHandler` のロールオーバー・再オープン動作を活用できる
