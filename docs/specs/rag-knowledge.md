...
| --- | --- | --- | --- |
| web | リンク集 URL | `url_pattern`（正規表現） | 既存の rag_crawl と同等。リンク集から URL を発見し一括取り込み |
...
| `--identifier` | 単一取り込み時 | ソース識別子（URL、ファイルパス等）。`--batch` と同時指定不可 |
| `--batch` | いいえ | 一括取り込みモード。指定時は `--source` が必須、`--identifier` と同時指定不可 |
| `--source` | 一括取り込み時 | 発見元（ユーザー名、ディレクトリパス等）。`--batch` 指定時のみ有効 |
...
ローカルファイルシステム上のファイルを読み込み、ナレッジベースに取り込むインジェスター。
静的なドキュメント（職務経歴書、学習ノート等）をリポジトリから直接取り込むユースケースを想定する。

#### 対応フォーマット

テキストとして読み込み可能なファイルを対象とする。
...
対応拡張子以外のファイルが指定された場合はバリデーションエラーとする。

#### 単一ファイル取り込み

ファイルパスを識別子として受け取り、バリデーション後にファイル内容を読み込む。タイトルはファイル名（拡張子除去）から導出する。読み込んだ内容を中間データモデルに変換して返却する。

ソースタイプは `local_file` とし、ソース識別子にはファイルの絶対パスを使用する。

#### ディレクトリ一括取り込み（discover）

ディレクトリパスと glob パターンを受け取り、対象ファイルを自動発見する。

- glob パターン未指定時は、指定ディレクトリ直下の対応拡張子ファイルを全て対象とする
- サブディレクトリを含めて探索する場合は `**` を含む glob パターンを明示的に指定する
- 発見されたファイルのパスリストを返し、一括取り込みパイプラインに渡す

#### 識別子のバリデーション

以下を検証し、正規化済みの絶対パスを返す:

- パスを正規化（シンボリックリンク解決・`..` 展開）した上で、許可ディレクトリ配下であることを検証する
- ファイルが存在すること
- 通常ファイルであること（ディレクトリ・デバイスファイル等は拒否）
- 拡張子が対応フォーマットに含まれること
- シンボリックリンクの場合はリンク先を解決した上で、リンク先の実体ファイルに対して上記の検証を行う

バリデーション失敗時はエラーを返す。

#### セキュリティ制約

<!-- How追加理由: PR #90 の Copilot レビューで任意パス読み取りによる機密ファイル漏洩リスクが指摘されたため -->

ローカルファイルの読み取りはパストラバーサル攻撃のリスクがある。MCP クライアントやエージェントからの入力は信頼できない可能性があるため、許可ディレクトリによるアクセス制限を設ける。

- 許可ディレクトリを環境変数で設定する（複数指定可）
- 未設定時はファイル取り込みを拒否する（フェイルクローズ）
- バリデーション時にパスを正規化し、許可ディレクトリ配下であることを検証する
- `..` やシンボリックリンクによる許可ディレクトリ外へのエスケープを防止する

#### 制約

- 文字コードは UTF-8 のみ対応。デコードエラー時はそのファイルをスキップしエラーを記録する
- シンボリックリンクはリンク先の実体ファイルを対象とする。リンク先が存在しない場合はエラーとする
- ファイルサイズの上限は設けない。大規模ファイルは後段のチャンキングで適切に分割される
...
Zenn API 経由で記事を取得するインジェスター。Zenn は動的ページ生成のため URL クロールでは本文テキストを取得できず、API 利用が必須である。

#### 識別子

記事スラッグ（記事 URL のパス末尾部分）を識別子として使用する。

バリデーション:

- 英小文字・数字・ハイフンのみ許容する
- 大文字が含まれる場合は小文字に正規化する
- 空文字列・不正な文字を含む場合はエラーを返す

#### 単一記事取得（fetch_single）

スラッグを指定して記事 API から本文（Markdown 形式）を取得する。取得した本文と記事タイトルを中間データモデルに変換して返す。

`source_type` は `zenn` とし、識別子（`source_id`）にはスラッグを設定する。メタデータには記事の公開日時等の付加情報を含める。

本文が空の記事（下書き状態等）は取り込みをスキップし、`None` を返す。

#### 記事一覧の発見（discover）

ユーザー名を指定して記事一覧 API からスラッグのリストを取得する。ページネーションに対応し、全ページを順次取得する。

ページネーション制御:

- 次ページの有無をレスポンスに含まれるページネーション情報で判定する
- 次ページが存在しない場合に取得を終了する
- ページネーション上限を環境変数で設定可能とし、上限到達時は取得済み分のスラッグを返す（安全弁）

#### レート制限対策

<!-- How追加理由: Issue #73 で約5,000件リクエストにより Cloudflare IP ブロックが発生したため、制約として明記 -->

Zenn API は公式のレート制限仕様を公開していない。過去にレート制限未考慮で大量リクエストを送信した結果、Cloudflare による IP ブロックが発生した。この教訓に基づき、以下の対策を必須とする。

リクエスト間ディレイ:

- 連続するリクエスト間に一定のディレイを挿入する
- ディレイ値は環境変数で設定可能とする（未設定時はデフォルト値を適用）
- discover のページネーションリクエストにもディレイを適用する

429 ステータスハンドリング:

- HTTP 429（Too Many Requests）を受信した場合、指数バックオフでリトライする
- リトライ上限を設け、上限到達時はエラーを返す
- `Retry-After` ヘッダーが存在する場合はその値を尊重する

#### セッション管理

HTTP セッションはリクエストごとに生成・破棄する（短命セッション方式）。リクエストタイムアウトを設定し、無応答時のハングを防止する。

ユーザーエージェントを設定し、Bot としての識別を明示する。

#### バッチ取り込みフロー

```mermaid
flowchart LR
    USER["ユーザー名"] --> DISC["discover: 記事一覧取得"]
    DISC --> SLUGS["スラッグリスト"]
    SLUGS --> FETCH["fetch_single を順次実行"]
    FETCH --> IC["中間データモデル"]
    IC --> PIPE["チャンキング・保存"]
```

バッチ取り込みでは discover で取得したスラッグリストに対して fetch_single を順次実行する。各リクエスト間にはレート制限ディレイを適用する。個別の記事取得に失敗した場合はその記事をスキップし、他の記事の処理を続行する。
...
| robots.txt の Disallow | クロールをスキップする。取得失敗時はフェイルオープンでクロールを許可する（Web ソースのみ） |
...
| ローカルファイルの拡張子が未対応 | バリデーションエラーを返す。バッチ時は他のファイルの処理を続行する |
| ローカルファイルが空（0 バイト） | 空のテキストとして取り込む。チャンク生成は後段のチャンカーの判断に委ねる |
| シンボリックリンクのリンク切れ | ファイルが存在しない場合と同様にエラーを返す |
| ローカルファイルの読み取り権限なし | エラーを返す。バッチ時は他のファイルの処理を続行する |
| ローカルファイルのパスが許可ディレクトリ外 | バリデーションエラーを返す。`..` やシンボリックリンクで許可外に到達する場合も同様 |
| ローカルファイルの許可ディレクトリ未設定 | ファイル取り込みを拒否する（フェイルクローズ） |
| discover で対象が 0 件 | 取り込み対象なしのメッセージを返す。エラーとしては扱わない |
| Zenn API から 429 応答 | 指数バックオフでリトライする。リトライ上限到達時はエラーを返す |
| Zenn API から 404 応答（記事取得） | 記事が存在しない旨のエラーを返す。バッチ時は他の記事の処理を続行する |
| Zenn API から 5xx 応答 | サーバーエラーとして処理し、`None` を返す。バッチ時は他の記事の処理を続行する |
| Zenn 記事の本文が空 | 取り込みをスキップし、`None` を返す。バッチ時は他の記事の処理を続行する |
| Zenn API タイムアウト | タイムアウトエラーとして処理し、`None` を返す。バッチ時は他の記事の処理を続行する |
| Zenn ユーザーが存在しない | discover が空リストを返す。エラーとしては扱わない |
