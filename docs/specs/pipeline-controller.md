...
- **バッチサイズ無制限**: 各ファイルを順次処理しメモリに全件を保持しないため、バッチサイズ制限は設けない。外部 API 通信も行わないため、大量ファイルの一括処理でも外部リソースへの影響はない
...
| `R`（リネーム） | パス変更 | 新パスで変換 | 旧パス削除 + 新パス追加 | ソース更新（パス。local 媒体は source_id も再導出） |
...
| .meta ファイルのみが変更された場合 | metadata.db のメタデータを更新する。コンバーター・インデクサーの再処理は行わない（データ本体に変更がないため） |
