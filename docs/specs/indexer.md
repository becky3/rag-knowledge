...
2. metadata.db から source_id に対応するメタデータを取得する
3. コンテンツタイプを検出する（テーブル/見出し付きテキスト/散文）
4. コンテンツタイプに応じたチャンキング戦略でテキストを分割する
5. 各チャンクにメタデータを付与する
6. Embedding プロバイダーでベクトルを生成する
7. ChromaDB に upsert する
8. BM25 インデックスに追加する
9. 更新の場合、旧チャンクのうち新チャンクに含まれないもの（stale chunks）を削除する

### メタデータ更新の処理手順

.meta サイドカーファイルのみが変更された場合（データ本体に変更がない場合）に実行される。

1. metadata.db から source_id に対応する更新後のメタデータを取得する
2. ChromaDB から source_id に紐づく全チャンクの ID を取得する
3. 各チャンクのメタデータを更新後の値で upsert する（`title` 等の変更を反映）
4. チャンクの再生成・Embedding の再計算は行わない
