...
3. コンテンツタイプに応じたチャンキング戦略でテキストを分割する。チャンカーは本文テキストと構造情報（見出し階層等）を返す
4. 各チャンクに呼び出し元から受け取ったメタデータを付与する。構造情報から `section_path` を生成しメタデータに含める
...
| `section_path` | str | 見出し階層（`>` 区切り、現セクションを含む）。見出しチャンカー・テーブルチャンカーが生成する。散文チャンカーでは空文字列。例: `第1章 データ処理 > 1.1 前処理 > 1.1.1 正規化` |
