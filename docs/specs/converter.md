...
- **変換結果は UTF-8 テキスト**: 全ての変換結果は UTF-8 エンコーディングのテキストファイルとして出力する
...
| PDF のテキスト抽出結果が空 | 空の Markdown ファイルを生成し、警告ログを出力する |
...
| パススルー対象ファイルの読み込みエラー（文字コード不正等） | エラーログを出力し、当該ファイルの変換をスキップする |
