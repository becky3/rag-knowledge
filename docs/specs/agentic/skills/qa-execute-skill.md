...
QA 検証ステップの実行・確認を、エージェントの自己判断でスキップされないように制御するスキル。L3 本番相当 QA を実行する `/qa` スキルの内部スキルとして動作する。
...
- [QA 戦略](../../workflows/qa-strategy.md) — 3 レイヤー設計と本スキルの位置づけ（L3 配下の検証ステップ実行制御）
