...
外部 API 通信の想定プロファイルは、各媒体の既存仕様書を継承する。各媒体のインジェスターは独立した MCP ツールから個別に呼び出されるため、複数媒体が同時に外部 API 通信を行うシナリオは発生しない。インジェスターの責務変更（source_store 配置のみ）により、後続のチャンキング・インデックス構築のリクエストはなくなるが、データ取得部分のリクエスト特性は変わらない。
...
各インジェスターが提供する操作。全操作の出力は source_store へのファイル配置であり、パイプライン制御への取り込み完了通知を含む。
...
ツール入力パラメータ・出力形式・バリデーション規則は各媒体の既存仕様書を継承する。
...
生成タイミング: データファイルの配置直後に同階層に .meta を生成する。データファイルと .meta は常にペアで存在する。
...
| ケース | 振る舞い |
|--------|---------|
| source_store ディレクトリが存在しない場合 | パイプライン制御が初回実行時にディレクトリと git リポジトリを初期化する。インジェスターはディレクトリの存在を前提とし、存在しない場合はエラーを返す |
...
| 同一 source_id に対する並行実行 | MCP ツールは逐次実行されるため、同一 source_id への並行書き込みは発生しない |
