...
- 複数ファイル形式（Markdown、プレーンテキスト、PDF）への対応。AsciiDoc は #174 完了後に追加予定
...
| HTTP モードでのデフォルト無効 | 運用安全 | HTTP モード時は明示的 opt-in が必要 | デフォルト変更不可 |
...
パターンのバリデーション: `pattern` に `..` や絶対パス（`/` 始まり）が含まれる場合はバリデーションエラーとして拒否する。加えて、glob マッチ結果の各ファイルを `Path.resolve()` で正規化した後、`dir_path` の配下であることを検証し、配下でないファイルは除外する。
...
| `.adoc` | **計画中（#174 に依存）**: AsciiDoc としてそのまま取り込む予定。チャンカーの AsciiDoc モード対応（#174）完了後に有効化する |

対応拡張子は環境変数 `RAG_LOCAL_SUPPORTED_EXTENSIONS` で追加可能（セクション「設定項目」参照）。追加した拡張子のファイルはプレーンテキストとして取り込む。

> **Note**: `.adoc` は #174（チャンカーの AsciiDoc モード対応）完了後に正式対応する。#174 完了までは初期デフォルトから除外し、`.md,.txt,.pdf` の 3 形式をデフォルトとする。
...
| 環境変数 | 型 | デフォルト | 説明 |
|---------|-----|-----------|------|
| `RAG_LOCAL_SUPPORTED_EXTENSIONS` | 文字列 | `".md,.txt,.pdf"` | 対応ファイル拡張子のカンマ区切りリスト。先頭にドット（`.`）を含める。`.adoc` は #174 完了後に追加予定 |
...
5. ファイル形式に応じたテキスト抽出を行う（テキストファイルは UTF-8 エンコーディングで読み取る）:
   ...
   - `.adoc`: #174 完了後に対応予定（現在は未対応拡張子として扱う）
...
3. ディレクトリの存在を確認する
4. `pattern` をバリデーションする（`..` や絶対パスを含む場合は拒否）
...
| `pattern` に `..` や絶対パスが含まれる | バリデーションエラーとして拒否する |
...
> **TODO:#174** `.adoc` ファイルの取り込みはチャンカーの AsciiDoc モード対応（#174）に依存する。#174 完了後にデフォルト拡張子へ `.adoc` を追加し、対応ファイル形式の表から「計画中」の注記を除去する。
