...
**同一ターン制約:** qa-execute から結果（OK/NG/SKIP）が返った場合、そのレスポンス内で即座に次の `/qa-execute` の Skill 呼び出しを行うこと。
qa-execute の step 6 が継続/中止の判断を完結させているため、qa スキル側で追加のユーザー確認やテキスト出力を単独で行ってはならない
（テキスト出力のみのレスポンスはターンを終了させ、ユーザー入力待ちになるため）。
結果の記録が必要な場合は、次の Skill 呼び出しと同一レスポンスに含める。STOP または最終ステップ完了時のみ、テキスト出力でターンを終了してよい。
