...
**同一ターン制約:** qa-execute の結果を受け取った後、次のステップが残っている場合は、結果の記録と次の `/qa-execute` の Skill 呼び出しを**同一レスポンス内**で行うこと。テキスト出力のみのレスポンスはターンを終了させユーザー入力待ちになるため、中間テキスト（ステップ結果の記録等）と次の Skill 呼び出しを必ず同じレスポンスに含める。STOP または最終ステップ完了時のみ、テキスト出力でターンを終了してよい。
